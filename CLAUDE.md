...
- **🚫 NO ASSUMPTIONS — verify before asserting.** Standing rule added 11 Aug 2026 after three failures: a claim that security headers existed on nourkrinsa.co.za (they did not, and the note would have suppressed a real finding on a live payments site), a Google Play policy date stated as fact that no primary source could confirm, and confirming Darius's recollection about client email setup from notes alone when his own WhatsApp screenshot showed the opposite. **If a claim can be checked with a grep, a file read, or an API call, check it.** These memory files are what a previous session believed, not ground truth — they have been wrong; the live system beats them. Never write an unverified claim into them as fact; label it verified-how-and-when or UNVERIFIED. Do not confirm Darius's memory just because it is plausible — say what would settle it. Always check the live state before anything client-facing or that costs money. Wrong answers here reach clients and contracts; slower is cheaper than wrong.
